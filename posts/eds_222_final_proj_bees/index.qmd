...
My first question is simply if there is a significant effect of pesticide use on bee biodiversity. The model can be written as:
...
4. If spatial autocorrelation is present, create an error model.
...
This initial analysis found that there was no significant correlation between biodiversity and pesticide use. There are a variety of possible reasons which I will discuss in a later section. 

Because biodiversity and pesticide use was not significantly correlated, I want to add more variables to see if there is potentially omitted variable bias. The variables I chose are:
* Population density, due to it's implicit connection with development.
* habitat fragmentation
* and land cover, added as a binary variable between habitat and non-habitat cover.


<u>From here</u>
1. I ran several more linear models to check for correlations, in which each new variable was added to the original linear model individually.
2. The variable that was most correlative with the lowest p-value was land cover, so I decide to test for spatial autocorrelation again, and add to my error model. Our new model is:

$$\text{Species Richness} = {\beta}_0 + {\beta}_1\text{Pesticide Amount} + {\beta}_2\text{Land Cover}$$
...
<figcaption>A histogram of a normal distribution for the randomization test. The difference in y intercept was obtained 1000 times when randomly shuffling the land cover variable.</figcaption>
</br>
5. Accept or reject the null hypothesis

Because the p-value is significantly below p = 0.05, we can reject the null.

This is obviously possibly an erroneous result, as it's extremely unlikely for our t-statistic to be so far outside the null distribution.
